...
module.exports = {
    ...
    createCommunity: async function (user, params) {
        ...
        const communityPayload = {
            ...
            community_number: params.communityNumber ? params.communityNumber : "",
            org_community_number: params.communityNumber ? params.communityNumber : "",
            payment_category: params.paymentCategory,
            non_profit: params.paymentCategory === "NonProfit"
        };
        console.log(communityPayload,"communityPayload................");
    ...
    communityStatusChange: async function (communityId) {
        ...
        if (community.is_active == true) {
            ...
            if (Lib.isEmpty(user.selected_community)) {
    ...
    approveOrRejectMemberRequest: async function (data) {
        ...
        if (memberDetails) {
            ...
            fcmToken = memberDetails.device_details.filter(device => device.is_active === true).map(device => device.fcm_token).filter(token => token !== null && token !== undefined);
    ...
    removeCommunityMember: async function (communityId, memberId) {
        try {
            const community = await Communities.findOne({
                _id: ObjectId(communityId),
                is_active: true,
    ...
    async communityMemberStatusChange(communityId, memberId) {
        const community = await Communities.findOne({
            _id: ObjectId(communityId),
            is_active: true,
            is_deleted: false
    ...
    publicityPageStatusChange: async function (communityId) {
