...
module.exports = {
    // Mutations
    updateHomePageOverview: async function (id, params) {
    ...
    updateCommunityFeaturedStatus: async function (id) {
        const community = await Communities.findOne({
            _id: ObjectId(id)
        });
        if (Lib.isEmpty(community)) {
    ...
    updatefreezePaneStatus: async (id) => {
    ...
    updateEventPaymentStatus: async (id) => {
        const communitySettings = await CommunitySettings.findOne({ community_id: id });
    ...
    updateCommunityAnnouncementSettings: async function (id, params) {
        const community = await Communities.findOne({
            _id: ObjectId(id)
        });
        if (Lib.isEmpty(community)) {
