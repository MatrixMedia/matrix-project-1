...
module.exports = {
    ...
    discoverGroupList: async function(communityId, userId, search=null, page = 1, limit=10) {
        ...
        return {error:false, message:"generalSuccess", data:{
    ...
    updateGroup: async function(id,params,UserId) {
        try {
            ...
            if(user.user_type == 'admin' || groupDetails.created_by == UserId){
                let group = await Group.update({ _id: ObjectId(id)}, { "$set":GroupObj});
                return ({error: false, message: "groupUpdateSuccess", data: group});
            }else{
    ...
    addGroupMember: async function (params) {
        // try {
